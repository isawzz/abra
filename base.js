...
function mItem(id, diDOM, di = {}) {
	let item = di;
	id = isdef(id) ? id : isdef(diDOM.div) && !isEmpty(diDOM.div.id) ? diDOM.div.id : getUID();
	if (isdef(diDOM.div)) diDOM.div.id = id;
...
function mSize(d, w, h, unit = 'px', sizing) { mStyleX(d, { width: w, height: h }, unit); if (isdef(sizing)) setRect(d, sizing); }
